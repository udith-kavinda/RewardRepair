...
def syntactic(epoch,syn_train_data_path):  
    ...
    if epoch == 0 and 'pretrain' in syn_train_data_path:
        ...
        model = T5ForMultiSourceConditionalGeneration(config).to(device)
        ...
        model = T5ForMultiSourceConditionalGeneration.from_pretrained(SAVE_MODEL, output_hidden_states=True)    
