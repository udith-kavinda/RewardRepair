...
class PLBartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states_1 is not None:
            ...
            # if(past_key_value is not None): print('past_key_value',len(past_key_value))

            # --------------------------1st---------------------------------------
            # cross_attn cached key/values tuple is at positions 3,4 of present_key_value tuple
            cross_attn_past_key_value = past_key_value[2:4] if past_key_value is not None else None
            hidden_states, cross_attn_weights_1, cross_attn_present_key_value = self.encoder_attn_1(
                hidden_states=hidden_states,
            ...
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = residual + hidden_states
            hidden_states = self.encoder_attn_layer_norm_1(hidden_states)
            ...
            hidden_states, cross_attn_weights_2, cross_attn_present_key_value = self.encoder_attn_2(
                hidden_states=hidden_states,
            ...
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = residual + hidden_states
            hidden_states = self.encoder_attn_layer_norm_2(hidden_states)
        ...
        if output_attentions:
            outputs += (self_attn_weights, cross_attn_weights_1, cross_attn_weights_2)
