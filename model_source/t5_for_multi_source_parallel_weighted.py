...
class T5BlockDecoder(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            expected_num_past_key_values = 2 if encoder_hidden_states_1 is None else 4
        ...
        if do_cross_attention:
            ...
            cross_attention_outputs_1 = self.layer[1](
                hidden_states,
            ...
            cross_attention_outputs_2 = self.layer[2](
                hidden_states,
            ...
            if(len(cross_attention_outputs_1)>3 or len(cross_attention_outputs_2)>3):
                raise ValueError("len(cross_attention_outputs)>3")

            hidden_states = torch.add(cross_attention_outputs_1[0]*0.9, cross_attention_outputs_2[0]*0.1)
            # hidden_states = torch.cat((cross_attention_outputs_1[0], cross_attention_outputs_2[0]), dim=2)
            ...
            if present_key_value_state is not None:
                present_key_value_state = present_key_value_state + (torch.add(cross_attention_outputs_1[1][0]*0.9, cross_attention_outputs_2[1][0]*0.1),
                                                                     torch.add(cross_attention_outputs_1[1][1]*0.9, cross_attention_outputs_2[1][1]*0.1))

                # present_key_value_state = present_key_value_state + (torch.cat((cross_attention_outputs_1[1][0], cross_attention_outputs_2[1][0]), dim=2),
                #                                                      torch.cat((cross_attention_outputs_1[1][1], cross_attention_outputs_2[1][1]), dim=2))
            # Keep cross-attention outputs and relative position weights
            # attention_outputs = attention_outputs + cross_attention_outputs[2:]

            attention_outputs = attention_outputs + (torch.add(cross_attention_outputs_1[2]*0.9, cross_attention_outputs_2[2]*0.1),)
            # attention_outputs = attention_outputs + (torch.cat((cross_attention_outputs_1[2], cross_attention_outputs_2[2]),dim=3),)

        # Apply Feed Forward layer
        hidden_states = self.layer[-1](hidden_states)
        # print('hidden_states ff.shape', hidden_states.shape)
...
class T5StackDecoder(T5PreTrainedModel):
    ...
    def forward(
        ...
        if self.model_parallel:
            raise ValueError("model_parallel=True")
...
class T5ForMultiSourceParallelConditionalGeneration(T5PreTrainedModel):
